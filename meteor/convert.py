...
def temp_dwpt_to_rhum(row: dict):
    ...
    return 100 * (
        math.exp(
            (17.625 * row['dwpt']) / (243.04 + row['dwpt'])
        ) / math.exp(
            (17.625 * row['temp']) / (243.04 + row['temp'])
        )
    ) if row['temp'] is not None and row['dwpt'] is not None else None
...
def pres_to_msl(row: dict, altitude: int = None, temp: str = 'tavg'):
    ...
    try:
        return None if (
            ...
            row['pres'] * math.pow(
                (1 - ((0.0065 * altitude) / (row[temp] + 0.0065 * altitude + 273.15))),
                -5.257
            ),
        1)
