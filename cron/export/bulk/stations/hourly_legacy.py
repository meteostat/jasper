...
class Task(Meteor):
    """
    Export hourly station obs data (legacy)
    """

    MODE = argv[1] # 'all' or 'recent'
    name = f'export.bulk.hourly.legacy.{MODE}'  # Task name
    ...
    def main(self) -> None:
		...
		FROM (
			...
			WHERE
				`station` = :station
                {f'AND `time` BETWEEN "{start_year}-01-01 00:00:00" AND "{end_year}-12-31 23:59:59"' if self.MODE == 'recent' else ''}
			)
		UNION ALL
			(SELECT
				`time`,
