...
class Task(Meteor):
    """
    Export meta data for all weather stations
    """

    name = 'export.bulk.stations.meta'  # Task name
    ...
    def main(self) -> None:
        ...
        if result.rowcount > 0:
            ...
            for record in data:
                ...
                if (
                    record[14] is not None or
                    record[16] is not None or
                    record[18] is not None or
                    record[20] is not None or
                    record[22] is not None
                ):
                    lite.append(station_object)
                    # Convert to list
                    record = record.values()
                    # Add slim rows
                    slim_cols = [0, 1, 3, 4, 6, 7, 8, 9,
                                 10, 11, 15, 16, 17, 18, 19, 20]
                    slim.append([record[i] for i in slim_cols])

            # Write JSON dumps
            self.export_json(full, '/stations/full.json.gz')
            self.export_json(lite, '/stations/lite.json.gz')

            # Write CSV dumps
            self.export_csv(slim, '/stations/slim.csv.gz')
            self.export_csv([row[0:13]
                            for row in slim], '/stations/lib.csv.gz')
