...
jsp = Jasper("export.bulk.raw.model")

# Get weather station(s)
stations = get_stations(jsp, read_file("model_stations.sql"), STATIONS_PER_CYCLE)

# Export data for each weather station
for station in stations:
    try:
        result = jsp.query(
            read_file("model.sql"), {"station": station[0]}
        )
            ...
            for year in range(first_year, last_year + 1):
                try:
                    ...
                    if len(d) > 0:
                        # Export data dump
                        export_csv(
                            jsp, [list(result.keys())] + d, f"/raw/model/{year}/{station[0]}.csv.gz"
                        )
                except:
                    pass
    except:
