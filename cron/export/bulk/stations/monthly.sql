...
FROM (
  ...
  (
    SELECT
      YEAR(CONVERT_TZ(`hourly_model`.`time`, "UTC", :timezone)) AS `year`,
      MONTH(CONVERT_TZ(`hourly_model`.`time`, "UTC", :timezone)) AS `month`,
      IF(
        COUNT(`hourly_model`.`temp`) < DAY(
          LAST_DAY(
            DATE(CONVERT_TZ(`hourly_model`.`time`, "UTC", :timezone))
          )
        ) * 24,
        NULL,
        ROUND(AVG(`hourly_model`.`temp`), 1)
      ) AS `tavg`,
      NULL AS `tmin`,
      NULL AS `tmax`,
      IF(
        COUNT(`hourly_model`.`prcp`) < DAY(
          LAST_DAY(
            DATE(CONVERT_TZ(`hourly_model`.`time`, "UTC", :timezone))
          )
        ) * 24,
        NULL,
        ROUND(SUM(`hourly_model`.`prcp`), 1)
      ) AS `prcp`,
      NULL AS `snow`,
      IF(
        COUNT(`hourly_model`.`wdir`) < DAY(
          LAST_DAY(
            DATE(CONVERT_TZ(`hourly_model`.`time`, "UTC", :timezone))
          )
        ) * 24,
      ...
      IF(
        COUNT(`hourly_model`.`wspd`) < DAY(
          LAST_DAY(
            DATE(CONVERT_TZ(`hourly_model`.`time`, "UTC", :timezone))
          )
        ) * 24,
        NULL,
        ROUND(AVG(`hourly_model`.`wspd`), 1)
      ) AS `wspd`,
      IF(
        COUNT(`hourly_model`.`wpgt`) < DAY(
          LAST_DAY(
            DATE(CONVERT_TZ(`hourly_model`.`time`, "UTC", :timezone))
          )
        ) * 24,
        NULL,
        ROUND(MAX(`hourly_model`.`wpgt`), 1)
      ) AS `wpgt`,
      IF(
        COUNT(`hourly_model`.`pres`) < DAY(
          LAST_DAY(
            DATE(CONVERT_TZ(`hourly_model`.`time`, "UTC", :timezone))
          )
        ) * 24,
        NULL,
        ROUND(AVG(`hourly_model`.`pres`), 1)
      ) AS `pres`,
      NULL AS `tsun`,
      "C" AS `priority`
    FROM
