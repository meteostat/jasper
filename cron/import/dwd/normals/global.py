...
from meteor.schema import normals_global


class Task(Meteor):
    """
    Import climate normals from DWD
    """

    name = 'import.dwd.normals.global'
    # dev_mode = True  # Running in dev mode?

    ENDPOINT = (
        'http://opendata.dwd.de/climate_environment/CDC/' +
        'observations_global/CLIMAT/multi_annual/'
    )
    START = 1961
    END = 1990
    PARAMETERS = [
    ...
    def main(self) -> None:
        ...
        for parameter in self.PARAMETERS:
            # Read CSV
            df = pd.read_csv(
                f'{self.ENDPOINT}{parameter["dir"]}{self.START}_{self.END}.txt',
                sep=';',
                dtype={
                    'Station': 'object'
                }
        ...
        # Add start & end year
        df_full['start'] = self.START
        df_full['end'] = self.END

        # Write DataFrame into Meteostat database
        self.persist(df_full, normals_global)
