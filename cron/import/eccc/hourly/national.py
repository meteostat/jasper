...
from meteor.convert import pres_to_msl
from meteor.schema import hourly_national


class Task(Meteor):
    """
    Import hourly (national) data from ECCC
    """

    name = 'import.eccc.hourly.national'  # Task name
    ...
    def _load(self, station: str, year: int, month: int) -> pd.DataFrame():
        """
    ...
    def main(self) -> None:
        ...
        if len(datasets) > 1:
            ...
            if full.index.size > 0:
                # Write into database
                self.persist(full, hourly_national)
