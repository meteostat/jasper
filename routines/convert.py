...
def pres_to_msl(row: dict, altitude: int = None):
    try:
        return None if isnan(row['pres']) or isnan(row['tavg']) or isnan(altitude) or altitude is None or row['pres'] == -999 else row['pres'] * math.pow((1 - ((0.0065 * altitude) / (row['tavg'] + 0.0065 * altitude + 273.15))), -5.257)
    except:
        return None
